...
let rslice_materialize_desc t ~cmp ?from_ ?to_ () =
  (* Descending cursor walk with datom-level bounds (same predicates as the old
     to_list + filter path). Do not use incomplete bound keys as LMDB hi_key:
     prefix bounds use e=0/tx0 and would exclude matching datoms under key<=. *)
  let datoms = ref [] in
  Datascript_lmdb_db.fold_index_range_desc_until t.which t.db
    ~stop:(fun key value ->
      let datom = decode_entry t.which key value in
      (* Once we are at/under the upper bound, stop below the lower bound. *)
      let under_hi =
        match from_ with
        | None -> true
...
let rslice_seq ?from_ ?to_ ?cmp t =
  let cmp = Option.value ~default:(cmp_for t.which) cmp in
  (* Exact attr prefix: reverse the attr slice in memory (same datoms as forward
     fold_attr_exact_prefix) instead of a full-index descending walk. *)
  let datoms =
    match attr_exact_prefix from_ to_ t.which with
...
let fold_tave_range f init db ~from_tx ?to_tx ?attr () =
  ...
  let acc = ref init in
  Datascript_lmdb_db.fold_index_range_until Tave db ~from_key
    ~stop:(fun key _value ->
      match to_tx with
...
let prune_tave_before db ~before_tx =
  if before_tx < 0 then ()
    ...
    Datascript_lmdb_db.fold_index_range_until Tave db
    ...
    if !to_delete <> [] then
      Datascript_lmdb_db.with_write_txn db (fun txn ->
        List.iter
          (fun key -> Datascript_lmdb_db.remove_index_txn Tave txn db key)
          !to_delete)
