...
let test_storage_round_trips_db_state () =
  ...
  in
  store ~storage db;
  if storage_addresses storage <> [ "datascript/root"; "datascript/tail" ] then
    failwith "store should write root and empty tail addresses";
  match restore storage with
  | None -> failwith "restore should return a db after store"
  | Some restored ->
    assert_equal_datoms
      "storage restore preserves active datoms"
      (datoms db Eavt ())
      (datoms restored Eavt ());
    assert_equal_datoms
      "storage restore preserves history datoms"
      (datoms (history db) Eavt ())
      (datoms (history restored) Eavt ());
    if schema restored <> schema db then
      failwith "storage restore should preserve schema";
    if entid restored "name" (String "Petr") <> Some 1 then
      failwith "storage restore should preserve unique indexes"

let test_restore_attaches_storage_and_settings () =
  let backing = memory_storage () in
  let db =
    empty_db ~schema:[ "name", indexed ] ()
    |> db_with [ Add (Entity_id 1, "name", String "Ivan") ]
  in
  store ~storage:backing db;
  match restore backing with
  | None -> failwith "restore should return a db after store"
  | Some restored ->
    let attached =
      match storage restored with
      | None -> failwith "restored db should remember its storage"
      | Some attached -> attached
    in
    if List.assoc_opt "branching-factor" (settings restored) <> Some (Int 512) then
      failwith "settings should expose default branching factor";
    if List.assoc_opt "ref-type" (settings restored) <> Some (Keyword "soft") then
      failwith "settings should expose default ref type";
    if List.assoc_opt "storage" (settings restored) <> Some (Bool true) then
      failwith "settings should expose storage attachment";
    store_tail attached [ [ datom ~tx:(tx0 + 2) ~e:1 ~a:"name" ~v:(String "Petr") () ] ];
    (match restore attached with
     | None -> failwith "attached storage should still restore"
     | Some replayed ->
       assert_equal_triples
         "storage returned from restored db should be usable"
         [ 1, "name", String "Petr" ]
         (datoms replayed Avet ~a:"name" ()))

let test_store_uses_attached_storage () =
  let backing = memory_storage () in
  let db =
    empty_db ~schema:[ "name", indexed ] ()
    |> db_with [ Add (Entity_id 1, "name", String "Ivan") ]
  ...
  in
  let updated = db_with [ Add (Entity_id 1, "name", String "Petr") ] restored in
  store updated;
  match restore backing with
  | None -> failwith "attached store should preserve root"
  | Some replayed ->
    assert_equal_triples
      "store should use storage attached to the db"
      [ 1, "name", String "Petr" ]
      (datoms replayed Avet ~a:"name" ())

let test_empty_db_and_init_db_attach_storage () =
  let empty_storage = memory_storage () in
  let empty =
    empty_db ~schema:[ "name", indexed ] ~storage:empty_storage ()
    |> db_with [ Add (Entity_id 1, "name", String "Ivan") ]
  in
  store empty;
  (match restore empty_storage with
   | None -> failwith "empty_db attached storage should store"
   | Some restored ->
     assert_equal_triples
       "empty_db should attach storage"
       [ 1, "name", String "Ivan" ]
       (datoms restored Avet ~a:"name" ()));
  let init_storage = memory_storage () in
  let initial = datom ~e:2 ~a:"name" ~v:(String "Petr") () in
  let initialized = init_db ~schema:[ "name", indexed ] ~storage:init_storage [ initial ] in
  store initialized;
  match restore init_storage with
  | None -> failwith "init_db attached storage should store"
  | Some restored ->
    assert_equal_triples
      "init_db should attach storage"
      [ 2, "name", String "Petr" ]
      (datoms restored Avet ~a:"name" ())

let test_addresses_returns_attached_storage_addresses () =
  let first_storage = memory_storage () in
  let second_storage = memory_storage () in
  let first =
    empty_db ~schema:[ "name", indexed ] ~storage:first_storage ()
    |> db_with [ Add (Entity_id 1, "name", String "Ivan") ]
  in
  let second =
    empty_db ~schema:[ "name", indexed ] ~storage:second_storage ()
    |> db_with [ Add (Entity_id 2, "name", String "Petr") ]
  in
  store first;
  store second;
  first_storage.storage_store [ "first/stale", Storage_tail [] ] [];
  second_storage.storage_store [ "second/stale", Storage_tail [] ] [];
  if addresses [ empty_db () ] <> [] then failwith "addresses should ignore dbs without attached storage";
  if addresses [ first ] <> [ "datascript/root"; "datascript/tail"; "first/stale" ] then
    failwith "addresses should list addresses from the attached storage";
  if addresses [ first; first ] <> [ "datascript/root"; "datascript/tail"; "first/stale" ] then
    failwith "addresses should deduplicate addresses";
  if addresses [ first; second ] <> [ "datascript/root"; "datascript/tail"; "first/stale"; "second/stale" ] then
    failwith "addresses should return the union of attached storage addresses"

let test_storage_restore_missing_root_returns_none () =
  match restore (memory_storage ()) with
  | None -> ()
  | Some _ -> failwith "restore should return None when storage has no root"

let test_storage_restore_replays_tail_datoms () =
  let storage = memory_storage () in
  let db =
    empty_db ~schema:[ "block/updated-at", indexed ] ()
    |> db_with [ Add (Entity_id 1, "block/updated-at", Int 1) ]
  in
  store ~storage db;
  store_tail
    storage
    [ [ datom ~tx:(tx0 + 1) ~e:1 ~a:"block/updated-at" ~v:(Int 2) () ]
    ; [ datom ~tx:(tx0 + 2) ~e:1 ~a:"block/updated-at" ~v:(Int 3) () ]
    ];
  match restore storage with
  | None -> failwith "restore should return a db after store"
  | Some restored ->
    assert_equal_triples
      "restore should replay tail groups over the stored root"
      [ 1, "block/updated-at", Int 3 ]
      (datoms restored Avet ~a:"block/updated-at" ());
    assert_equal_int "restore should advance max tx from tail groups" (tx0 + 2) restored.max_tx

let test_db_with_tail_replays_groups_and_skips_invalid_groups () =
  let db =
    empty_db ~schema:[ "block/updated-at", indexed; "block/uuid", unique_identity ] ()
    |> db_with
         [ Add (Entity_id 1, "block/updated-at", Int 1)
         ; Add (Entity_id 1, "block/uuid", String "u1")
         ]
  in
  let restored =
    db_with_tail
      db
      [ [ datom ~tx:(tx0 + 2) ~e:1 ~a:"block/updated-at" ~v:(Int 2) () ]
      ; [ datom ~tx:(tx0 + 3) ~e:2 ~a:"block/uuid" ~v:(String "u1") ()
        ; datom ~tx:(tx0 + 3) ~e:2 ~a:"block/title" ~v:(String "Title") ()
        ]
      ; [ datom ~tx:(tx0 + 4) ~e:3 ~a:"block/title" ~v:(String "Later") () ]
      ]
  in
  assert_equal_triples
    "db_with_tail should replay valid groups and skip groups rejected by constraints"
    [ 1, "block/updated-at", Int 2
    ; 1, "block/uuid", String "u1"
    ; 3, "block/title", String "Later"
    ]
    (datoms restored Eavt ());
  assert_equal_int "db_with_tail should advance max tx through tail groups" (tx0 + 4) restored.max_tx

let test_storage_full_store_clears_stale_tail () =
  let storage = memory_storage () in
  let db =
    empty_db ~schema:[ "name", indexed ] ()
    |> db_with [ Add (Entity_id 1, "name", String "Ivan") ]
  in
  store ~storage db;
  store_tail storage [ [ datom ~tx:(tx0 + 2) ~e:1 ~a:"name" ~v:(String "Petr") () ] ];
  let compacted =
    db_with [ Add (Entity_id 1, "name", String "Oleg") ] db
  in
  store ~storage compacted;
  match restore storage with
  | None -> failwith "restore should return a db after compacted store"
  | Some restored ->
    assert_equal_triples
      "full store should clear stale tail data"
      [ 1, "name", String "Oleg" ]
      (datoms restored Avet ~a:"name" ())

let test_file_storage_round_trips_across_instances () =
  let dir =
    Filename.concat
  ...
  Fun.protect
    ...
    (fun () ->
      ...
      if storage_addresses restored_storage <> [ "datascript/root"; "datascript/tail" ] then
        failwith "file_storage should list persisted addresses";
      match restore restored_storage with
      | None -> failwith "file_storage should restore stored db"
      | Some restored ->
        assert_equal_triples
          "file_storage should restore root and replay persisted tail"
          [ 1, "name", String "Petr" ]
          (datoms restored Avet ~a:"name" ()))

let test_storage_collect_garbage_deletes_unreachable_addresses () =
  let storage = memory_storage () in
  let db =
    empty_db ~schema:[ "name", indexed ] ()
    |> db_with [ Add (Entity_id 1, "name", String "Ivan") ]
  ...
  if storage_addresses storage <> [ "datascript/root"; "datascript/tail" ] then
    failwith "collect_garbage should delete addresses not reachable from the stored db";
  match restore storage with
  | None -> failwith "collect_garbage should keep a restorable db"
  | Some restored ->
    assert_equal_triples
      "collect_garbage should keep root and tail payloads"
      [ 1, "name", String "Petr" ]
      (datoms restored Avet ~a:"name" ())
...
let test_entity_reads_current_attributes () =
  let db =
    ...
    |> db_with
         ...
         ; Retract (Entity_id 1, "aka", Some (String "IV"))
         ]
  in
  match entity db (Entity_id 1) with
  | None -> failwith "expected entity 1 to exist"
  | Some entity ->
    ...
    assert_equal_tx_value
      ...
      (Some (Many_values [ String "Terrible" ]))
      (entity_attr entity "aka");
    assert_equal_tx_value
      "missing attributes return none"
      None
      (entity_attr entity "missing")

let test_entity_exposes_db_id_attr () =
  let db =
    empty_db ()
...
let test_entity_exposes_source_db_and_touch () =
  let db =
    empty_db ()
    |> db_with [ Entity { db_id = Some (Entity_id 1); attrs = [ "name", One_value (String "Ivan") ] } ]
  in
  ...
  | Some entity ->
    ...
    assert_equal_tx_value
      "touch preserves loaded attributes"
      (Some (One_value (String "Ivan")))
      (entity_attr touched "name");
    assert_equal_datoms
...
let test_entity_ref_attrs_navigate_to_entities () =
  let db =
    ...
    |> db_with
         ...
         ; Add (Entity_id 1, "child", Ref 3)
         ; Add (Entity_id 1, "child", Ref 4)
         ; Add (Entity_id 2, "name", String "Petr")
         ; Add (Entity_id 3, "name", String "David")
         ; Add (Entity_id 4, "name", String "Thomas")
         ]
  in
  match entity db (Entity_id 1) with
  | None -> failwith "expected entity"
  | Some entity ->
    assert_equal_tx_value
      "entity ref attrs navigate to the target entity"
      (Some
         (One_entity
            { db_id = Some (Entity_id 2)
            ; attrs = [ "name", One_value (String "Petr") ]
            }))
      (entity_attr entity "friend");
    assert_equal_tx_value
      "entity many ref attrs navigate to target entities"
      (Some
         (Many_entities
            [ { db_id = Some (Entity_id 3); attrs = [ "name", One_value (String "David") ] }
            ; { db_id = Some (Entity_id 4); attrs = [ "name", One_value (String "Thomas") ] }
            ]))
      (entity_attr entity "child")

let test_touch_expands_component_refs () =
  let db =
    empty_db ~schema:[ "profile", component; "part", component_many; "friend", ref_attr ] ()
    |> db_with
         [ Add (Entity_id 1, "name", String "Ivan")
         ; Add (Entity_id 1, "profile", Ref 2)
         ; Add (Entity_id 1, "part", Ref 3)
         ; Add (Entity_id 1, "part", Ref 4)
         ; Add (Entity_id 1, "friend", Ref 5)
         ; Add (Entity_id 2, "email", String "ivan@example.com")
         ; Add (Entity_id 3, "name", String "Part A")
         ; Add (Entity_id 4, "name", String "Part B")
         ; Add (Entity_id 5, "name", String "Petr")
         ]
  in
  match entity db (Entity_id 1) with
  | None -> failwith "expected entity"
  | Some entity ->
    let touched = touch entity in
    assert_equal_tx_value
      "touch recursively expands cardinality-one component refs"
      (Some
         (One_entity
            { db_id = Some (Entity_id 2)
            ; attrs = [ "email", One_value (String "ivan@example.com") ]
            }))
      (entity_attr touched "profile");
    assert_equal_tx_value
      "touch recursively expands cardinality-many component refs"
      (Some
         (Many_entities
            [ { db_id = Some (Entity_id 3); attrs = [ "name", One_value (String "Part A") ] }
            ; { db_id = Some (Entity_id 4); attrs = [ "name", One_value (String "Part B") ] }
            ]))
      (entity_attr touched "part");
    assert_equal_tx_value
      "touch keeps non-component refs as ordinary navigable refs"
      (Some
         (One_entity
            { db_id = Some (Entity_id 5)
            ; attrs = [ "name", One_value (String "Petr") ]
            }))
      (entity_attr touched "friend")

let test_entity_drops_missing_ref_targets () =
  let db =
    empty_db
      ~schema:
        [ "ref", ref_attr
  ...
  | Some entity ->
    assert_equal_tx_value
      "cardinality-one missing ref target is omitted"
      None
      (entity_attr entity "ref");
    assert_equal_tx_value
      "missing component target is omitted"
      None
      (entity_attr entity "comp");
    assert_equal_tx_value
    ...
    assert_equal_tx_value
      "cardinality-many missing component targets are omitted"
      None
      (entity_attr entity "multicomp")

let test_entity_reverse_reference_attrs () =
  let db =
    empty_db ~schema:[ "child", many; "profile", component ] ()
    |> db_with
         ...
         ; Entity { db_id = Some (Entity_id 4); attrs = [ "profile", One_value (Ref 2) ] }
         ]
  in
  (match entity db (Entity_id 2) with
   | None -> failwith "expected entity 2 to exist"
   | Some entity ->
     assert_equal_tx_value
       "entity reverse attr navigates to incoming ref owners"
       (Some
          (Many_entities
             [ { db_id = Some (Entity_id 1)
               ; attrs =
       ...
       (entity_attr entity "_child");
     assert_equal_tx_value
       "entity reverse component attr navigates to the single owner"
       (Some
          (One_entity
...
let test_entity_reverse_reference_attrs_preserve_namespace () =
  let db =
    empty_db ~schema:[ "person/child", many ] ()
    |> db_with
         [ Entity { db_id = Some (Entity_id 1); attrs = [ "person/child", Many_values [ Ref 2 ] ] }
         ; Entity { db_id = Some (Entity_id 2); attrs = [ "name", One_value (String "David") ] }
         ]
  in
  match entity db (Entity_id 2) with
  ...
  | Some entity ->
    assert_equal_tx_value
      "entity reverse attr preserves attr namespace"
      (Some
         (Many_entities
            [ { db_id = Some (Entity_id 1)
              ; attrs = [ "person/child", Many_values [ Ref 2 ] ]
              }
            ]))
      (entity_attr entity "person/_child")

let test_entity_missing_entity () =
  let db = empty_db () in
  if entity db (Entity_id 42) <> None then failwith "missing entity should return None"

let test_entity_requires_forward_datoms () =
  let db =
    empty_db ()
    |> db_with [ Add (Entity_id 1, "friend", Ref 2) ]
  in
  if entity db (Entity_id 2) <> None then
    failwith "incoming refs alone should not make an entity exist";
  if pull db [ Pull_reverse_ref ("friend", [ Pull_attr "friend" ]) ] (Entity_id 2) <> None then
    failwith "pull should not fabricate reverse-only entities"
  ...
  let db =
    ...
    |> db_with
         [ Entity
             ...
             ; attrs =
                 [ "name", One_value (String "Ivan")
                 ; "age", One_value (Int 32)
                 ]
             }
         ]
  in
  assert_equal_int
    "entid resolves unique identity attributes"
    1
    (match entid db "name" (String "Ivan") with
     | Some eid -> eid
     | None -> failwith "expected entid to resolve");
  assert_equal_triples
    "unique identity entity maps upsert into the existing entity"
...
let test_unique_identity_rejects_conflicting_explicit_entity_ids () =
  ...
  in
  let upsert_with_id db_id attrs =
    db_with [ Entity { db_id = Some db_id; attrs } ] db
  in
...
let test_unique_many_identity_upserts_from_any_value () =
  ...
  in
  let db =
    empty_db ~schema:[ "slugs", unique_many_identity ] ()
    |> db_with
         [ Entity
             { db_id = Some (Entity_id 1)
             ; attrs =
                 [ "name", One_value (String "Ivan")
                 ; "slugs", Many_values [ String "ivan1"; String "ivan2" ]
                 ]
             }
         ]
  in
  let report =
    transact
      ...
      [ Entity
          { db_id = None
          ; attrs =
              [ "slugs", Many_values [ String "ivan3"; String "ivan1" ]
              ; "age", One_value (Int 35)
              ]
          }
      ]
  in
  assert_equal_triples
    "cardinality many identity upsert scans every value"
    [ 1, "age", Int 35
    ; 1, "name", String "Ivan"
    ; 1, "slugs", String "ivan1"
    ; 1, "slugs", String "ivan2"
    ; 1, "slugs", String "ivan3"
    ]
    (datoms report.db_after Eavt ())
...
let test_add_tempid_upserts_by_unique_identity () =
  let report =
    empty_db ~schema:[ "name", unique_identity ] ()
    |> db_with [ Add (Entity_id 1, "name", String "Ivan") ]
    |> fun db ->
    transact
      db
      [ Add (Temp_id "ivan", "name", String "Ivan")
      ; Add (Temp_id "ivan", "age", Int 31)
      ]
  in
  assert_equal_triples
    "Add tempid unique identity upserts into existing entity"
    [ 1, "age", Int 31; 1, "name", String "Ivan" ]
    (datoms report.db_after Eavt ());
  assert_equal_tempids
    "tempid should resolve to existing unique identity entity"
...
let test_ref_attrs_do_not_upsert_tempids () =
  let db =
    empty_db ~schema:[ "name", unique_identity; "ref", ref_attr ] ()
    |> db_with [ Entity { db_id = None; attrs = [ "name", One_value (String "Alice") ] } ]
  in
  let report =
    transact
      db
      [ Entity { db_id = Some (Temp_id "target"); attrs = [ "name", One_value (String "Igor") ] }
      ; Entity
          { db_id = Some (Temp_id "person")
          ; attrs = [ "name", One_value (String "Anna"); "ref", One_value (Ref_to (Temp_id "target")) ]
          }
      ]
  in
  assert_equal_triples
    "ref attrs should resolve tempids without upserting by referenced entity id"
    [ 1, "name", String "Alice"
    ; 2, "name", String "Igor"
    ; 3, "name", String "Anna"
    ; 3, "ref", Ref 2
    ...
    (datoms report.db_after Eavt ());
  assert_equal_tempids
    "ref attr tempids should resolve to newly allocated entities"
    [ "db/current-tx", tx0 + 2; "target", 2; "person", 3 ]
    report.tempids

let test_tempid_retry_preserves_explicit_entity_ids () =
  let db =
    empty_db ~schema:[ "name", unique_identity; "ref", ref_attr ] ()
    |> db_with
         [ Entity { db_id = None; attrs = [ "name", One_value (String "Alice") ] }
         ; Entity { db_id = None; attrs = [ "name", One_value (String "Bob") ] }
         ]
  in
  let report =
    transact
      db
      [ Entity { db_id = Some (Entity_id 3); attrs = [ "ref", One_value (Ref_to (Temp_id "A")) ] }
      ; Entity { db_id = Some (Entity_id 4); attrs = [ "ref", One_value (Ref_to (Temp_id "B")) ] }
      ; Entity { db_id = Some (Temp_id "A"); attrs = [ "name", One_value (String "Alice") ] }
      ; Entity { db_id = Some (Temp_id "B"); attrs = [ "name", One_value (String "Bob") ] }
      ]
  in
  assert_equal_triples
    "tempid retry remaps refs without clobbering explicit ids"
    [ 1, "name", String "Alice"
    ; 2, "name", String "Bob"
    ; 3, "ref", Ref 1
    ; 4, "ref", Ref 2
    ...
    (datoms report.db_after Eavt ());
  assert_equal_tempids
    "retried tempids resolve to their upsert targets"
    [ "db/current-tx", tx0 + 2; "A", 1; "B", 2 ]
    report.tempids

let test_add_tempids_retry_unique_identity_upserts () =
  let db =
    empty_db ~schema:[ "name", unique_identity ] ()
    |> db_with
         [ Add (Temp_id "a", "age", Int 42)
         ; Add (Temp_id "b", "likes", String "Pizza")
         ; Add (Temp_id "a", "name", String "Bob")
         ; Add (Temp_id "b", "name", String "Bob")
         ]
  in
  assert_equal_triples
    "tempids with the same unique identity value are merged in one transaction"
    [ 1, "age", Int 42; 1, "likes", String "Pizza"; 1, "name", String "Bob" ]
    (datoms db Eavt ())

let test_add_tempid_unique_identity_conflicting_upserts () =
  let db =
    empty_db ~schema:[ "name", unique_identity ] ()
    |> db_with
         [ Add (Temp_id "ivan", "name", String "Ivan")
  ...
  in
  assert_raises_invalid_arg
    "one tempid cannot upsert into two existing unique identity entities"
    (fun () ->
      ignore
        (db_with
           [ Add (Temp_id "person", "name", String "Ivan")
           ; Add (Temp_id "person", "age", Int 35)
           ; Add (Temp_id "person", "name", String "Oleg")
           ; Add (Temp_id "person", "age", Int 36)
           ]
           db))
...
let test_storage_backed_connections_restore_and_continue () =
  let storage = memory_storage () in
  let conn = create_conn ~schema:[ "name", unique_identity ] ~storage () in
  if storage_addresses storage <> [ "datascript/root"; "datascript/tail" ] then
    failwith "storage-backed create_conn should store the initial root and empty tail";
  ignore (transact_conn conn [ Add (Entity_id 1, "name", String "Ivan") ]);
  ignore (transact_conn conn [ Add (Entity_id 2, "name", String "Petr") ]);
  let restored =
    match restore_conn storage with
    | Some conn -> conn
    | None -> failwith "restore_conn should return a conn after create_conn stores root"
  in
  assert_equal_triples
    "restore_conn should replay transaction tail"
    [ 1, "name", String "Ivan"; 2, "name", String "Petr" ]
    (datoms (conn_db restored) Eavt ());
  ignore (transact_conn restored [ Add (Entity_id 3, "name", String "Oleg") ]);
  match restore storage with
  | None -> failwith "storage should still contain a db"
  | Some db ->
    assert_equal_triples
      "restored storage-backed conn should keep persisting later transactions"
...
let test_storage_backed_connections_honor_skip_store_metadata () =
  ...
  ignore (transact_conn ~tx_meta:[ "skip-store?", Bool true ] conn [ Add (Entity_id 1, "name", String "Ivan") ]);
  assert_equal_triples
    "skip-store transactions should still update the live conn"
    [ 1, "name", String "Ivan" ]
    (datoms (conn_db conn) Eavt ());
  (match restore storage with
   ...
   | Some db ->
     assert_equal_triples
       "skip-store transactions should not be persisted to storage"
       []
       (datoms db Eavt ()));
  ignore (transact_conn conn [ Add (Entity_id 2, "name", String "Petr") ]);
  (match restore storage with
   ...
   | Some db ->
     assert_equal_triples
       "later stored transactions should not persist earlier skipped tx data"
       [ 2, "name", String "Petr" ]
       (datoms db Eavt ()))

let test_storage_backed_conn_from_db_and_datoms_store_initial_root () =
  let from_db_storage = memory_storage () in
  let from_db =
    empty_db ~schema:[ "name", indexed ] ~storage:from_db_storage ()
    |> db_with [ Add (Entity_id 1, "name", String "Ivan") ]
  in
  let from_db_conn = conn_from_db from_db in
  (match restore from_db_storage with
   | None -> failwith "conn_from_db should store dbs with attached storage"
   | Some restored ->
     assert_equal_triples
       "conn_from_db stores the initial attached db root"
       [ 1, "name", String "Ivan" ]
       (datoms restored Eavt ()));
  ignore (transact_conn from_db_conn [ Add (Entity_id 2, "name", String "Petr") ]);
  (match restore from_db_storage with
   | None -> failwith "conn_from_db storage-backed conn should remain restorable"
   | Some restored ->
     assert_equal_triples
       "conn_from_db keeps persisting later transactions"
       [ 1, "name", String "Ivan"; 2, "name", String "Petr" ]
       (datoms restored Eavt ()));
  let from_datoms_storage = memory_storage () in
  let from_datoms_conn =
    conn_from_datoms
      ~schema:[ "name", indexed ]
      ~storage:from_datoms_storage
      [ datom ~e:3 ~a:"name" ~v:(String "Oleg") () ]
  in
  (match restore from_datoms_storage with
   | None -> failwith "conn_from_datoms should store initial datoms with attached storage"
   | Some restored ->
     assert_equal_triples
       "conn_from_datoms stores the initial attached db root"
       [ 3, "name", String "Oleg" ]
       (datoms restored Eavt ()));
  ignore (transact_conn from_datoms_conn [ Add (Entity_id 4, "name", String "Dima") ]);
  match restore from_datoms_storage with
  | None -> failwith "conn_from_datoms storage-backed conn should remain restorable"
  | Some restored ->
    assert_equal_triples
      "conn_from_datoms keeps persisting later transactions"
...
let () =
  ...
  test_empty_db_and_init_db_attach_storage ();
  test_addresses_returns_attached_storage_addresses ();
  test_storage_restore_missing_root_returns_none ();
  test_storage_restore_replays_tail_datoms ();
  test_db_with_tail_replays_groups_and_skips_invalid_groups ();
  test_storage_full_store_clears_stale_tail ();
  test_file_storage_round_trips_across_instances ();
  test_storage_collect_garbage_deletes_unreachable_addresses ();
  ...
  test_entity_reverse_reference_attrs ();
  test_entity_reverse_reference_attrs_preserve_namespace ();
  test_entity_missing_entity ();
  test_entity_requires_forward_datoms ();
