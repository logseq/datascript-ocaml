...
let test_eval_arithmetic () =
  assert_equal_value_option
    ...
    (Some (Float 3.5))
    (Built_ins.eval_arithmetic AddNumbers [ Int64 1L; Float 2.5 ]);
  assert_equal_value_option
    "division returns an integer when the quotient is integral"
    (Some (Int64 2L))
    (Built_ins.eval_arithmetic DivideNumbers [ Int64 4L; Int64 2L ]);
  assert_equal_value_option
    "division returns a float when the quotient is fractional"
    (Some (Float 2.5))
    (Built_ins.eval_arithmetic DivideNumbers [ Int64 5L; Int64 2L ]);
  assert_equal_value_option
    "modulo follows Clojure sign semantics"
    (Some (Int64 1L))
    (Built_ins.eval_arithmetic ModuloNumbers [ Int64 (-3L); Int64 2L ]);
  assert_raises_invalid_arg "integer arithmetic rejects floats" (fun () ->
    ignore (Built_ins.eval_arithmetic RemainderNumbers [ Float 3.0; Int64 2L ]))
...
let test_extremum_value () =
  ...
  assert_equal_value
    ...
    (Int64 9L)
    (Built_ins.extremum_value MaximumValue (Int64 3L) [ Int64 9L; Int64 4L ])
...
let test_aggregate_result () =
  let values =
    [ Result_value (Int64 1L); Result_value (Int64 2L); Result_value (Int64 2L); Result_value (Float 3.5) ]
  in
  assert_equal_result "count counts all rows" (Result_value (Int64 4L)) (Built_ins.aggregate_result Count values);
  assert_equal_result
  ...
  assert_equal_result
    ...
    (Result_value (Set [ Int64 1L; Int64 2L; Float 3.5 ]))
  ...
  assert_equal_result
    ...
    (Result_value (Tuple [ Some (Int64 2L); Some (Float 3.5) ]))
