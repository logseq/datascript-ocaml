...
upstream_datascript_js="${OUTLINER_DATASCRIPT_JS:-${UPSTREAM_DATASCRIPT_JS:-$repo_root/../datascript/release-js/datascript.js}}"
...
if [ "${OUTLINER_BENCH_SKIP_BUILD:-0}" != "1" ]; then
  dune build --profile release bench/outliner_insert_ocaml.exe
fi
