...
module Make (Context : sig
  ...
  let search_datoms db index ?e ?a ?v ?tx () =
    Db.search_datoms db_index_context db index ?e ?a ?v ?tx ()

  let fold_datoms f init db index ?e ?a ?v ?tx () =
    Db.fold_datoms f init db_index_context db index ?e ?a ?v ?tx ()

  let datoms_list db index ?e ?a ?v ?tx () =
    Db.datoms_list db_index_context db index ?e ?a ?v ?tx ()

  let avet_datoms_by_value db attr value =
    Db.avet_datoms_by_value db_index_context db attr value
